...
export default function ForgotPassword() {
  ...
  useEffect(() => {
    ...
    const forgotPasswordParam = searchParams.get('forgotPassword');
    ...
    if (modeParam) {
      setMode(modeParam);
    } else if (forgotPasswordParam !== null) {
      setMode(forgotPasswordParam === 'true' ? 'forgotPassword' : 'verification');
    }
  }, [searchParams]);
  ...
  const getTitle = () => {
    switch (step) {
      case 'email':
        return mode === 'verification' ? 'Xác thực tài khoản' : 'Quên mật khẩu';
      case 'verification':
      ...
      case 'email':
        return (
          <form onSubmit={handleSendEmail} className="w-full space-y-4">
            ...
            >
              <Mail size={18} />
              {mode === 'verification' ? 'Gửi mã xác thực' : 'Gửi email đặt lại'}
            </Button>
          </form>
        );
      ...
      case 'verification':
        return (
          <form onSubmit={handleVerifyCode} className="w-full space-y-4">
            ...
            >
              <CheckCircle size={18} />
              {mode === 'verification' ? 'Xác thực tài khoản' : 'Xác nhận mã'}
            </Button>
            <Button 
              type="button"
              variant="outline"
